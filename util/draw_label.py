...
if __name__ == '__main__':
    drawlabel = DrawLabel()
    fp = 'E:\Draw_Label\Draw_Label-master\dataset\恶性\img_origin\\080124587肖仁定2.gif'
    drawlabel.read_gif(fp)
    drawlabel.draw_edge()
    drawlabel.save_edge()
    drawlabel.fill_label(2)
    ...
    plt.show()
    mpimg.imsave(fname='E:\Draw_Label\Draw_Label-master\dataset\恶性\img_label\\080124587肖仁定2.png',
                       arr=drawlabel.img_label)
    for i in drawlabel.edge_dict.keys():
        if drawlabel.img_label[i,:].sum() == 0:
            print(i)
    print(type(drawlabel.img_origin))
    print(type(drawlabel.img_label))
